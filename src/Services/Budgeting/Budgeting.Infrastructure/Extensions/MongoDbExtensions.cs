...
using DrifterApps.Holefeeder.Budgeting.Infrastructure.Exceptions;
using DrifterApps.Holefeeder.Framework.SeedWork;
using MongoDB.Driver.Linq;

namespace DrifterApps.Holefeeder.Budgeting.Infrastructure.Extensions
{
    public static class MongoDbExtensions
    {
        ...
        public static IMongoQueryable<T> Offset<T>(this IMongoQueryable<T> query, int offset) =>
            offset != 0 ? query.Skip(offset) : query;

        public static IMongoQueryable<T> Limit<T>(this IMongoQueryable<T> query, int limit) =>
            limit < int.MaxValue ? query.Take(limit) : query;

        private static IMongoQueryable<T> OrderBy<T>(this IMongoQueryable<T> source, string propertyName,
            bool isAscending) =>
            source.OrderByThenBy("OrderBy", propertyName, isAscending);

        private static IMongoQueryable<T> ThenBy<T>(this IMongoQueryable<T> source, string propertyName,
            bool isAscending) =>
            source.OrderByThenBy("ThenBy", propertyName, isAscending);

        private static IMongoQueryable<T> OrderByThenBy<T>(this IMongoQueryable<T> source, string method,
            string propertyName, bool isAscending)
        {
            ...
            var propertyInfo = source.ElementType.GetProperty(propertyName,
                BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);
            if (propertyInfo is null)
                throw new HolefeederInfrastructureException(propertyName);
        ...
        private static IMongoQueryable<T> Where<T>(this IMongoQueryable<T> source, string propertyName, string op,
            string value)
        {
            var parameter = Expression.Parameter(source.ElementType, string.Empty);

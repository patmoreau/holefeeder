...
namespace DrifterApps.Holefeeder.Budgeting.Infrastructure.Repositories
{
    public class CashflowQueriesRepository : ICashflowQueriesRepository
    {
        ...
        public Task<QueryResult<CashflowViewModel>> FindAsync(Guid userId, QueryParams queryParams,
        ...
        private async Task<QueryResult<CashflowViewModel>> FindInternalAsync(Guid userId, QueryParams queryParams)
        {
...
WHERE row_nb BETWEEN @offset AND @limit";
            ...
            return new QueryResult<CashflowViewModel>(count, _mapper.Map<IEnumerable<CashflowViewModel>>(cashflows));
        }
...
";
            ...
            var cashflows = await connection
                .QueryAsync<CashflowEntity, AccountEntity, CategoryEntity, CashflowEntity>(
                    ...
                    splitOn: "id,id")
                .ConfigureAwait(false);
            return _mapper.Map<CashflowViewModel>(cashflows.First());
        }
    }
}

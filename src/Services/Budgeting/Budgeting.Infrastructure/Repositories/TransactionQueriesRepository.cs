...
namespace DrifterApps.Holefeeder.Budgeting.Infrastructure.Repositories
{
    public class TransactionQueriesRepository : ITransactionQueriesRepository
    {
        ...
        public Task<QueryResult<TransactionViewModel>> FindAsync(Guid userId, QueryParams queryParams,
        ...
        private async Task<QueryResult<TransactionViewModel>> FindInternalAsync(Guid userId, QueryParams queryParams)
        {
...
WHERE row_nb BETWEEN @offset AND @limit";
            ...
            return new QueryResult<TransactionViewModel>(count,
...
";
            ...
            var transactions = await connection
                .QueryAsync<TransactionEntity, AccountEntity, CategoryEntity, TransactionEntity>(
                    ...
                    splitOn: "id,id")
                .ConfigureAwait(false);
            return _mapper.Map<TransactionViewModel>(transactions.First());
        }
    }
}

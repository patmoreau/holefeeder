...
export class ModifyCashflowComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.values$ = this.route.params.pipe(
      switchMap((params: Params) => this.cashflowsService.findById(params[cashflowIdParamName])),
      filterNullish(),
  ...
  onCancel() {
    this.modalService.delete(
      'Are you sure you want to cancel this cashflow?'
    ).pipe(
      filterTrue(),

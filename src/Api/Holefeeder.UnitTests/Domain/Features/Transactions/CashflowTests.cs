...
public class CashflowTests
{
    ...
    public CashflowTests(ITestOutputHelper testOutputHelper)
    {
    ...
    {
        // arrange
        var builder = GivenAnActiveCashflow().OfAmount(_faker.Finance.Amount(Decimal.MinValue, Decimal.MinusOne));
    ...
    public void GivenSetTags_WhenAddingTags_ThenTagListIsSet()
    {
        // arrange
        var cashflow = GivenAnActiveCashflow().Build();
        var newTags = _faker.Lorem.Words(_faker.Random.Int(1, 10)).Distinct().ToArray();
    ...
    public void GivenGetUpcoming_WhenInactiveCashflow_ThenReturnEmptyList()
    {
        ...
        // act
        var result = cashflow.GetUpcoming(_faker.Date.Future());

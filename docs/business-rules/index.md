...
Every entity (account, cashflow, category, store item, transaction) is scoped to the owning user. All queries and mutations filter by the authenticated user's ID — cross-user access is rejected at
every step.
